angular.module('kotei')
    .service('infoService', (jwtHelper, $http, $q, $moment, serviceUtils) => {
        ...
        return {
            ...
            getTrainingsByDateAndType: (from, to, trainingTypeIds, trainingCategoryIds) => get(`/api/training/hash?where={"$and":[{"from":{"$gte":"${from}"}},{"to":{"$lte":"${to}"}},{"training_type_id":{"\$or":[${trainingTypeIds.length ? trainingTypeIds.join(',') : 'null'}]}},{"training_category_id":{"\$or":[${trainingCategoryIds.join(',')}]}}]}`),
